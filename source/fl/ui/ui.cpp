...
#define PRINT_SPEED_VEC3(NAME, VECTOR) PRINT_3FLOAT(NAME, "H", VECTOR.x, "V", VECTOR.y, "S", VECTOR.z)
#define PRINT_POLAR_VEC2(NAME, VECTOR) PRINT_2FLOAT(NAME, "R", VECTOR.x, "θ", DEG(VECTOR.y))
#define PRINT_SPHERICAL_VEC3(NAME, VECTOR) PRINT_3FLOAT(NAME, "R", VECTOR.x, "θ", DEG(VECTOR.y), "φ", DEG(VECTOR.z))
#define PRINT_FVS_VEC3(NAME, VECTOR) PRINT_3FLOAT(NAME, "F", VECTOR.x, "V", VECTOR.y, "S", VECTOR.z)
#define PRINT_HV_VEC2(NAME, VECTOR) PRINT_2FLOAT(NAME, "H", DEG(VECTOR.x), "V", DEG(VECTOR.y))
#define PRINT_ABC_VEC3(NAME, VECTOR) PRINT_3FLOAT(NAME, "A", VECTOR.x, "B", VECTOR.y, "C", VECTOR.z)

#define SQ(NUM) NUM * NUM
#define PAD(NUM) al::isPadHold(-1, 1<<NUM)

//reduces between 0 and 2 pi
float reduceAngle(float ang) {
    while (ang < 0)
        ang += M_PI * 2;
    while (ang > M_PI * 2)
        ang -= M_PI * 2;
...
//reduces betweeen - and + pi
float reduceAnglePi(float ang) {
    while (ang < -M_PI)
        ang += M_PI * 2;
    while (ang > M_PI)
        ang -= M_PI * 2;
...
sead::Vector3f calculateGrowth(sead::Vector3f oldVec, sead::Vector3f newVec) {
    sead::Vector3f growth;
    growth.x = newVec.x / oldVec.x;
    growth.y = newVec.y / oldVec.y;
    growth.z = newVec.z / oldVec.z;
    return growth;
}
...
sead::Vector3f transformToRelativeAxes(sead::Vector3f vec, bool isTransVector, sead::Vector3f relativeTrans, float relativeAngle) {
    if (isTransVector) {
...
//fix to be ccw
sead::Vector3f transformByAngleCW(sead::Vector3f vec, double angle) {
    double zxAngle = atan2f(vec.x, vec.z);
    double caAngle = zxAngle - angle;
    double magnitude = sqrt(vec.z * vec.z + vec.x * vec.x);
    vec.z = magnitude * cosf(caAngle);
    vec.x = magnitude * sinf(caAngle);
    return vec;
...
sead::Vector3f transformEulerAnglesToRelativeAxes(sead::Vector3f vec, float relativeAngle) {
...
fl::ui::PracticeUI::LiveActorInfo::LiveActorInfo(al::LiveActor* actor, bool isPlayer, bool observedVelocities, const LiveActorInfo& old) {
    trans = *al::getTrans(actor);
    vel = *al::getVelocity(actor);
    if (observedVelocities) {
        vel = trans - old.trans;
    }
    accel = vel - old.vel;
    decay.x = vel.x / old.vel.x;
    decay.y = vel.y / old.vel.y;
    decay.z = vel.z / old.vel.z;
    
    hSpeed = sqrt(SQ(vel.x) + SQ(vel.z));
    speed = sqrt(SQ(vel.x) + SQ(vel.y) + SQ(vel.z));
    hVelAngle = reduceAngle(atan2f(vel.x, vel.z));
    //hVelAngle = reduceAngle(atan2f(vel.z, vel.x));
    vVelAngle = atan2f(vel.y, hSpeed); //do not reduce angle because negative is fine
    
    hVel = {hSpeed, hVelAngle};
    deltaHVel = hVel - old.hVel;
    deltaHDecay.x = hVel.x / old.hVel.x;
    deltaHDecay.y = deltaHVel.y;

    sphericalVel = {speed, hVelAngle, vVelAngle};
    deltaSphericalVel = sphericalVel - old.sphericalVel;

    quat = al::getQuat(actor);
    eulerAngles = fl::QuatToEulerRHR(quat);
    //eulerAngles = fl::QuatToEuler(quat);
    rot = {DEG(eulerAngles.x), DEG(eulerAngles.y), DEG(eulerAngles.z)};
    angVel = eulerAngles - old.eulerAngles;
    angVel.x = reduceAnglePi(angVel.x);
    angVel.y = reduceAnglePi(angVel.y);
    angVel.z = reduceAnglePi(angVel.z);
    angAccel = angVel - old.angVel;

    if (isPlayer) {
        PlayerActorHakoniwa* player = (PlayerActorHakoniwa*) actor;
        anim = player->mAnimator->mCurrentAnim;
        animFrame = player->mAnimator->getAnimFrame();
        animFrameMax = player->mAnimator->getAnimFrameMax();
    }  

//#if SMOVER == 100
    nerveName = strdup(PracticeUI::instance().getNerveName(actor));
    nerveStep = 0;
    al::NerveKeeper* playerNerveKeeper = actor->getNerveKeeper();
    if (playerNerveKeeper) {
        nerveStep = playerNerveKeeper->mStep;
    }
//#endif
}

#if SMOVER == 100
const char* fl::ui::PracticeUI::getNerveName(al::LiveActor* actor) {
    al::NerveKeeper* playerNerveKeeper = actor->getNerveKeeper();   
    char* nerveName;         
    if (playerNerveKeeper) {                                              
        int nerveStep = playerNerveKeeper->mStep;                            
        al::Nerve* nerve = (al::Nerve*)playerNerveKeeper->getCurrentNerve(); 
        if (nerve) {                                                  
            RTTI_WRAP(nerveWrap, nerve);                               
            char mangledNerveName[64] = "_Z";                          
            strcat(mangledNerveName, nerveWrap->GetRtti()->name);      
            nerveName = DEMANGLE(mangledNerveName);
            char* abbrevNerveName = strstr(nerveName, "Nrv");
            if (abbrevNerveName && strlen(abbrevNerveName) > 3) {
                nerveName = abbrevNerveName + 3;
            }
        }
    }
    return nerveName;
...
fl::ui::PracticeUI::CameraInfo::CameraInfo(const CameraInfo& old) {
    sead::LookAtCamera* camera = al::getLookAtCamera(PracticeUI::instance().stageScene, 0);
    pos = camera->mPos;
    at = camera->mAt;
    up = camera->mUp;
    diff = at - pos;
    ...
    ang = {atan2f(diff.x, diff.z), atan2f(diff.y, sqrtf(SQ(diff.x) + SQ(diff.z)))};
    angVel = ang - old.ang;
    angVel.x = reduceAnglePi(angVel.x);
    angVel.y = reduceAnglePi(angVel.y);
    angAccel = angVel - old.angVel;
...
fl::ui::PracticeUI::InputInfo::InputInfo(const InputInfo& old) {
    fl::TasHolder& h = fl::TasHolder::instance();

    left = *al::getLeftStick(CONTROLLER_AUTO);
    right = *al::getRightStick(CONTROLLER_AUTO);
    ...
    leftPolar = {sqrtf(SQ(left.x) + SQ(left.y)), reduceAngle(atan2f(left.y, left.x))};
    rightPolar = {sqrtf(SQ(right.x) + SQ(right.y)), reduceAngle(atan2f(right.y, right.x))};

    sead::LookAtCamera* camera = al::getLookAtCamera(PracticeUI::instance().stageScene, 0);
    sead::Vector3f vec = camera->mAt - camera->mPos;
    float camAng = atan2f(vec.y, sqrtf(SQ(vec.x) + SQ(vec.z)));
    
    leftPolarAbsolute = {leftPolar.x, reduceAngle(leftPolar.y + camAng - M_PI / 2)};
}
    ...
    if (showMenu) {
        ...
        switch (curPage) {
            ...
            case Info: {
                ...
                CHANGE_PAGE("Relative Player Info", InfoRelative, 2);
                CHANGE_PAGE("Cap Bounce Info", InfoCapBounce, 3);
                CHANGE_PAGE("Jump Info", InfoJump, 4);
                CHANGE_PAGE("Moon Info", InfoMoon, 5);
                CHANGE_PAGE("Capture Info", InfoHack, 6);
                CHANGE_PAGE("Cappy Info", InfoCappy, 7);
                CHANGE_PAGE("Statistics Info", InfoStatistics, 8);
                CHANGE_PAGE("Camera Info", InfoCamera, 9);
                //CHANGE_PAGE("Mighty Info 1", InfoMighty1, 8);
                //CHANGE_PAGE("Mighty Info 2", InfoMighty2, 9);
                TOGGLE("Quaternion Rotation", quatRot, 10);
                TOGGLE("Observed Velocities", observedVelocities, 11);
                TOGGLE("Show Spherical Velocities", sphericalVel, 12);
                TOGGLE("Show Delta Velocities", deltaVel, 13);
                MAX_LINE(14);
                break;
            }
            case InfoPlayer: {
                ...
                sead::Vector2f& leftStick = *al::getLeftStick(CONTROLLER_AUTO);
                float leftStickAngle = reduceAngle(atan2f(leftStick.y, leftStick.x));
                ...
                float cameraHorizontalAngle = reduceAngle(atan2f(diff.x, diff.z));

                float relativeLeftStickAngle = reduceAngle(leftStickAngle + cameraHorizontalAngle - M_PI / 2);

                static LiveActorInfo mInfoOld;
                LiveActorInfo mInfo(player, true, observedVelocities, mInfoOld);
                mInfoOld = mInfo;

                static LiveActorInfo cInfoOld;
                LiveActorInfo cInfo(cappy, false, observedVelocities, cInfoOld);
                cInfoOld = cInfo;

                PRINT_VEC3("Pos", mInfo.trans);
                PRINT_VEC3("Vel", mInfo.vel);
                if (deltaVel) {
                    PRINT_VEC3("Accel", mInfo.accel);
                    PRINT_VEC3("Decay", mInfo.decay);
                }
                PRINT_POLAR_VEC2("H Vel", mInfo.hVel);
                if (deltaVel) {
                    PRINT_POLAR_VEC2("Delta H Vel", mInfo.deltaHVel);
                    PRINT_POLAR_VEC2("Delta H Decay", mInfo.deltaHDecay);
                }
                if (sphericalVel) {
                    PRINT_SPHERICAL_VEC3("Vel", mInfo.sphericalVel);
                    if (deltaVel) {
                        PRINT_SPHERICAL_VEC3("Delta Vel", mInfo.deltaSphericalVel);
                    }
                }
                if (quatRot)
                    PRINT_QUATPTR("Rot", mInfo.quat);
                else {
                    PRINT_VEC3("Rot", mInfo.rot);
                    PRINT_FLOAT("Ang Vel", DEG(mInfo.angVel.y));
                    //PRINT_FLOAT("Ang Accel", mInfo.angAccel.y);
                }
                //PRINT_VEC3PTR("Bubble Pos", playerRecoveryPoint);
                printf(" Animation: %s (%.0f/%.0f)\n", mInfo.anim, mInfo.animFrame, mInfo.animFrameMax);

                //PRINT_FLOAT("Left Stick Angle", DEG(leftStickAngle));
                //PRINT_FLOAT("Camera Angle", DEG(cameraHorizontalAngle));
                //PRINT_FLOAT("Stick Direction", DEG(relativeLeftStickAngle));
                
#if SMOVER == 100
                printf(" Nerve: %s (%d)\n", mInfo.nerveName, mInfo.nerveStep);                 
#endif

                float velGravityDir = dot(*al::getGravity(player), *al::getVelocity(player));
                float velNonGravityDir = sqrt(SQ(mInfo.speed) - SQ(velGravityDir));
                float velUpDir = dot(*al::getUp(player), *al::getVelocity(player));
                float velNonUpDir = sqrt(SQ(mInfo.speed) - SQ(velGravityDir));

                //PRINT_VEC3PTR("Gravity", al::getGravity(player));
                //PRINT_2FLOAT("Vel Gravity", "F", velGravityDir, "S", velNonGravityDir);

                //PRINT_VEC3PTR("Up", al::getUp(player));
                //PRINT_2FLOAT("Vel Up", "F", velUpDir, "S", velNonUpDir);

                TRIGGER("Heal Mario", 1, Game::instance().healMario());

                //cap bounce testing
                /*
                sead::Vector3f mFoot(mInfo.trans.x, mInfo.trans.y + 40, mInfo.trans.z);
                sead::Vector3f mBody(mInfo.trans.x, mInfo.trans.y + 75, mInfo.trans.z);
                sead::Vector3f mHead(mInfo.trans.x, mInfo.trans.y + 110, mInfo.trans.z);
                sead::Vector3f cCatch(cInfo.trans.x, cInfo.trans.y + 20, cInfo.trans.z);
                PRINT_3FLOAT("Mario Sensors", "Foot", mFoot.y, "Body", mBody.y, "Head", mHead.y);
                PRINT_VEC3("Cappy Pos", cInfo.trans);
                PRINT_FLOAT("Cappy Catch", cCatch);
                sead::Vector3f footCatch = mFoot - cCatch;
                float footCatchAngle = atan2f(footCatch.y, sqrt(SQ(footCatch.x) + SQ(footCatch.z)));
                PRINT_3FLOAT("Foot-Cappy Catch", "S", norm2(footCatch), "V", footCatch.y, "θ", DEG(footCatchAngle));
                PRINT_FLOAT("Dist Body-Cappy", norm2(mBody - cInfo.trans));
                PRINT_FLOAT("Dist Head-Cappy", norm2(mHead - cInfo.trans));
                */
                ...
                if (h.isRunning)
                    ...
                    printf(" TAS Frames %lu\n", h.frameCount);
                break;
            }
            case InfoRelative: {
                TITLE("Relative Player Info");
                BACK_PAGE(Info, 0);
                MAX_LINE(2);
                ...
                static sead::Vector3f relativeTrans = sead::Vector3f::zero;

                static bool fixedOrigin = false;
                TOGGLE("Fixed Origin/Rotation", fixedOrigin, 1);
                if (!fixedOrigin) {
                    relativeTrans = mInfo.trans;
                    relativeAngle = mInfo.eulerAngles.y; //in radians
                }

                PRINT_VEC3("Origin", relativeTrans);
                PRINT_FLOAT("Rotation", DEG(relativeAngle));

                printf("\n");

                sead::Vector3f playerABCTrans = transformToRelativeAxes(mInfo.trans, true, relativeTrans, relativeAngle);
                float originAngle = reduceAngle(atan2f(playerABCTrans.x, playerABCTrans.z) - M_PI);
                if (fixedOrigin) {
                    PRINT_ABC_VEC3("Pos", playerABCTrans);
                    PRINT_POLAR_VEC2("H Diff to Origin", sead::Vector2f(sqrt(SQ(playerABCTrans.x) + SQ(playerABCTrans.z)), originAngle));
                }
                PRINT_ABC_VEC3("Vel", transformToRelativeAxes(mInfo.vel, false, relativeTrans, relativeAngle));

                if (deltaVel) {
                    PRINT_ABC_VEC3("Accel", transformToRelativeAxes(mInfo.accel, false, relativeTrans, relativeAngle));
                }
                PRINT_POLAR_VEC2("H Vel", sead::Vector2f(mInfo.hVel.x, reduceAngle(mInfo.hVel.y - relativeAngle)));
                if (fixedOrigin) {
                    PRINT_FLOAT("H Vel to Origin", mInfo.hVel.x * cosf(originAngle - (mInfo.hVel.y - relativeAngle)));
                }
                if (deltaVel) {
                    PRINT_POLAR_VEC2("Delta H Vel", mInfo.deltaHVel);
                }
                if (sphericalVel) {
                    PRINT_SPHERICAL_VEC3("Vel", sead::Vector3f(mInfo.sphericalVel.x, reduceAngle(mInfo.sphericalVel.y - relativeAngle), mInfo.sphericalVel.z));
                    if (deltaVel) {
                        PRINT_SPHERICAL_VEC3("Delta Vel", mInfo.deltaSphericalVel);
                    }
                }
                sead::Vector3f playerABCEulerAngles = transformEulerAnglesToRelativeAxes(mInfo.eulerAngles, relativeAngle);
            ...
            case InfoJump: {
                ...
                sead::LookAtCamera* camera = al::getLookAtCamera(stageScene, 0);
                sead::Vector3f camVector = camera->mAt - camera->mPos;
                float cameraHorizontalAngle = reduceAngle(atan2f(camVector.x, camVector.z));
...
//#if SMOVER == 100
                const char* nerveName = getNerveName(player);
                int nerveStep = 0;
                al::NerveKeeper* playerNerveKeeper = player->getNerveKeeper();
                if (playerNerveKeeper) {
                    nerveStep = playerNerveKeeper->mStep;
                }
                static float initVelAngle;
                if (!(strcmp(nerveName, "Jump") == 0 || strcmp(nerveName, "Fall") == 0) || nerveStep == 0) {
                    if (mInfo.hVel.x > 0)
                        initVelAngle = mInfo.hVelAngle; //should make sure it uses real velocity but this is complicated on slopes
                    else
                        initVelAngle = mInfo.eulerAngles.y; //this breaks if mario is moving very slowly
                }
                sead::Vector3f fvsVel = transformByAngleCW(mInfo.vel, initVelAngle - M_PI / 2);
                sead::Vector3f fvsAccel = transformByAngleCW(mInfo.accel, initVelAngle - M_PI / 2);

                //function to make it stop alternating positive and negative when it would display 0
                //make this available for more vectors
                if (abs(fvsAccel.x) < 5 / pow(10, numDigits + 1)) {
                    fvsAccel.x = 0;
                }

                float absoluteLeftStickAngle = reduceAngle(leftStickAngle + cameraHorizontalAngle - M_PI / 2);
                float difference = reduceAnglePi(absoluteLeftStickAngle - initVelAngle);
                float forwardLeftStickAngle = reduceAngle(initVelAngle - cameraHorizontalAngle + M_PI / 2);
                float leftLeftStickAngle = reduceAngle(forwardLeftStickAngle + M_PI / 2);
                float rightLeftStickAngle = reduceAngle(forwardLeftStickAngle - M_PI / 2);

                PRINT_VEC3("Pos", mInfo.trans);
                PRINT_POLAR_VEC2("H Vel", mInfo.hVel);
                PRINT_FLOAT("Initial Velocity Angle", DEG(initVelAngle));
                PRINT_FVS_VEC3("Vel", fvsVel);
                PRINT_FVS_VEC3("Accel", fvsAccel);
                PRINT_FLOAT("Relative Stick Angle from IVA", DEG(difference));
                PRINT_3FLOAT("Left Stick Angles", "F", DEG(forwardLeftStickAngle), "L", DEG(leftLeftStickAngle), "R", DEG(rightLeftStickAngle));
                //eliminate the really stupid thing from above
//#endif
            ...
            case InfoCappy: {
                ...
                sead::Vector3f diff = cInfo.trans - mInfo.trans;
                sead::Vector2f mAngles = {atan2f(diff.x, diff.z), atan2f(diff.y, sqrtf(SQ(diff.x) + SQ(diff.z)))};

                PRINT_VEC3("Mario Pos", mInfo.trans);
                PRINT_SPEED("Diff from Mario", sqrt(SQ(diff.x) + SQ(diff.z)), diff.y, norm2(diff));
                PRINT_HV_VEC2("Angle from Mario", mAngles);
                PRINT_VEC3("Pos", cInfo.trans);
                PRINT_VEC3("Vel", cInfo.vel);
                PRINT_VEC3("Accel", cInfo.accel);
                PRINT_POLAR_VEC2("H Vel", cInfo.hVel);
                PRINT_POLAR_VEC2("Delta H Vel", cInfo.deltaHVel);
                if (sphericalVel) {
                    PRINT_SPHERICAL_VEC3("Vel", cInfo.sphericalVel);
                    PRINT_SPHERICAL_VEC3("Delta Vel", cInfo.deltaSphericalVel);
                }
                if (quatRot)
                    PRINT_QUATPTR("Rot", cInfo.quat);
                else {
                    PRINT_VEC3("Rot", cInfo.rot);
                    PRINT_FLOAT("Ang Vel", DEG(cInfo.angVel.y));
                    //PRINT_FLOAT("Ang Accel", cInfo.angAccel.y);
                }

                //printf(" Animation: %s (%.0f/%.0f)\n", cInfo.anim, cInfo.animFrame, cInfo.animFrameMax);

#if SMOVER == 100
                printf(" Nerve: %s (%d)\n", cInfo.nerveName, cInfo.nerveStep);                 
#endif
            ...
            case InfoCamera: {
                ...
                static CameraInfo cInfoOld;
                CameraInfo cInfo(cInfoOld);
                static InputInfo iInfoOld;
                InputInfo iInfo(iInfoOld);

                PRINT_VEC3("Up Direction", cInfo.up);
                PRINT_VEC3("Pos", cInfo.pos);
                PRINT_VEC3("Vel", cInfo.vel);
                PRINT_POLAR_VEC2("H Vel", cInfo.hVel);
                PRINT_POLAR_VEC2("Delta H Vel", cInfo.deltaHVel);
                PRINT_VEC3("Aimed At", cInfo.at);
                PRINT_VEC3("Difference from Target", cInfo.diff);
                PRINT_FLOAT("Distance to Target", cInfo.dist);
                PRINT_HV_VEC2("Angle", cInfo.ang);
                PRINT_HV_VEC2("Angle Vel", cInfo.angVel);
                PRINT_HV_VEC2("Angle Accel", cInfo.angAccel);
                PRINT_POLAR_VEC2("Right Stick", iInfo.rightPolar);
            ...
            case Tas: {
                ...
                TRIGGER("Connect to server", 1, {
                    smo::Server::instance().connect("192.168.1.100");
                });
                //TOGGLE("Old Motion Mod", fl::TasHolder::instance().oldMotion, 2);
                //TOGGLE("Absolute Stick Angles", absoluteTAS, 2);
                ...
                // printf(" Left Stick: (X: %.5f Y: %.5f)\n", iInfo.left.x, iInfo.left.y);
                // printf(" Right Stick: (X: %.5f Y: %.5f)\n", iInfo.right.x, iInfo.right.y);
                printf(" Left Stick Polar: (R: %.5f θ: %.5f)\n", DEG(reduceAngle(atan2f(iInfo.left.y, iInfo.left.x))), sqrt(iInfo.left.x * iInfo.left.x + iInfo.left.y * iInfo.left.y));
                printf(" Right Stick Polar: (R: %.5f θ: %.5f)\n", DEG(reduceAngle(atan2f(iInfo.right.y, iInfo.right.x))), sqrt(iInfo.right.x * iInfo.right.x + iInfo.right.y * iInfo.right.y));
	            printf(" Left Stick Cartesian: (X: %.0f Y: %.0f)\n", iInfo.left.x * 32767, iInfo.left.y * 32767);
	            printf(" Right Stick Cartesian: (X: %.0f Y: %.0f)\n", iInfo.right.x * 32767, iInfo.right.y * 32767);
                printf(" Buttons:\n");
                // -1 = CONTROLLER_AUTO
                printf(" %s %s %s %s %s %s %s %s\n", iInfo.a ? "A" : " ", iInfo.b ? "B" : " ", iInfo.x ? "X" : " ", iInfo.y ? "Y" : " ", iInfo.l ? "L" : " ", iInfo.r ? "R" : " ", iInfo.zl ? "ZL" : "  ", iInfo.zr ? "ZR" : "  ");
                printf(" %s %s %s %s %s %s %s %s\n", iInfo.plus ? "+" : " ", iInfo.minus ? "-" : " ", iInfo.ls ? "LS" : "  ", iInfo.rs ? "RS" : "  ", iInfo.dup ? "DUP" : "   ", iInfo.dright ? "DRIGHT" : "      ", iInfo.ddown ? "DDOWN" : "     ", iInfo.dleft ? "DLEFT" : "     ");
