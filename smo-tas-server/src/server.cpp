...
namespace smo
{
        ...
        OutPacketPlayerScriptData pD;
        {
            ...
            {         
                ...
                if (i >= 1500)
                {
                    ...
                    i = 0;
                    std::this_thread::sleep_for(std::chrono::milliseconds(script.frames.size() / 625));
                }
            }
            if (i != 0)
            {
    ...
    void smo::Server::loopThread()
    {
        ...
        const std::unordered_map<std::string, std::function<void(std::deque<std::string>&)>> cmd = {
            ...
            {"start", [this](std::deque<std::string>& args)
            {
                OutPacketPlayerScriptState p;
                p.state = 1;
                c.sendPacket(this, p, smo::OutPacketType::PlayerScriptState);
            }},
            {"st", [this](std::deque<std::string>& args)
            {
                OutPacketPlayerScriptState p;
                p.state = 1;
                c.sendPacket(this, p, smo::OutPacketType::PlayerScriptState);
            }},
            {"run", [this](std::deque<std::string>& args)
            {
                OutPacketPlayerScriptState p;
                p.state = 1;
                c.sendPacket(this, p, smo::OutPacketType::PlayerScriptState);
            }},
            {"r", [this](std::deque<std::string>& args)
            {
                OutPacketPlayerScriptState p;
                p.state = 1;
                c.sendPacket(this, p, smo::OutPacketType::PlayerScriptState);
            }},
            {"stop", [this](std::deque<std::string>& args)
            {
                OutPacketPlayerScriptState p;
                p.state = 0;
                c.sendPacket(this, p, smo::OutPacketType::PlayerScriptState);
